...
const StudentAccount = () => {
    ...
    const handleUpdateInfo = async () => {
        try {
            ...
            const res = await StudentService.updateStudent(decoded.id, data, student.accessToken)
            if (res?.status === "OK") {
        ...
        <div className='flex justify-center items-center h-full w-full flex-col'>
            <div className='animate-fade-in sm:w-[502px] w-full sm:h-max h-full backdrop bg-white bg-opacity-40 rounded sm:p-3 px-6 pt-2  border border-gray-300 shadow-lg'>
                <div action="#" className='flex flex-col gap-4 h-max py-6'>
                    ...
                    <div>
                        <label htmlFor="studentID" className="block sm:text-2xl text-xl font-montserrat leading-6 text-black">studentID</label>
                        <div className="mt-2">
                            <input value={student.studentID} id="studentID" name="studentID" placeholder='studentID' type="studentID" autoComplete="studentID" required className="font-palanquin block w-full rounded-md border-0 px-1.5 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-1 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" disabled />
                        </div>
                    </div>
                    <div>
                        <label htmlFor="email" className="block sm:text-2xl text-xl font-montserrat leading-6 text-black">email</label>
                        <div className="mt-2">
                            <input value={student.email} id="email" name="email" placeholder='email' type="email" autoComplete="email" required className="font-palanquin block w-full rounded-md border-0 px-1.5 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-1 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" disabled />
                        </div>
                    </div>
